...
import { projectApi, getApiErrorMessage } from "../../../lib/api";
...
const Projects = () => {
  ...
  useEffect(() => {
    ...
    projectApi
      .getAll()
      .then((data) => !cancelled && setProjectData(data))
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {!loading && !error && projectData.length === 0 && (
          <p className="text-center text-gray-500 text-sm">No projects yet.</p>
        )}

        {!loading && !error && projectData.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {projectData.map((project, index) => {
              const moduleId = `MODULE_${String(index + 1).padStart(2, "0")}`;
              const status = project.liveUrl ? "OPERATIONAL" : "IN_DEVELOPMENT";
              const deployUrl = project.liveUrl || project.githubUrl || "#";
              return (
                <motion.div
                  key={project._id}
